"""
Full Model with Rolling Average Features
========================================
Tests the model with the newly scraped data including rolling averages per team.
"""
...
def feature_engineering_rolling(df):
    """Features including H2H AND Rolling Averages."""
    ...
    # Drop rows where main odds are missing
    ...
    if 'odd_btts_yes' in df_clean.columns:
        features.append('odd_btts_yes')
    if 'odd_btts_no' in df_clean.columns:
        features.append('odd_btts_no')
    
    # STREAK FEATURES
    streak_cols = [c for c in df_clean.columns if c.startswith('streak_')]
    features.extend(streak_cols)
    
    # ROLLING AVERAGE FEATURES (NEW)
    # Identify columns like 'home_avg_ballPossession', 'away_avg_expectedGoals' etc.
    # Note: 'home_avg_height' and 'home_avg_rating' were already in base features from original scraper,
    # but our new rolling features share the 'avg_' pattern. 
    # Let's explicitly look for keys from the new feature engineering
    # We can just check for any numeric column starting with home_avg/away_avg that isn't already in features
    
    potential_rolling = [c for c in df_clean.columns if 'avg_' in c]
    for col in potential_rolling:
    ...
    # For rolling avgs, missing means 'not enough history'.
    # XGBoost handles NaNs, but filling with -1 or median is often safer for consistency in this pipeline.
    # Given other features use -1, we try that first.
...
if __name__ == "__main__":
    print("="*60)
    print("ROLLING AVERAGE MODEL (NEW FEATURES)")
    print("="*60)
    
    # Load NEW data
    df = load_and_preprocess_data("dataset_rolling_features.csv")
    print(f"Loaded {len(df)} matches")
    print(f"Date range: {df['date'].min().date()} to {df['date'].max().date()}")
    
    # Feature engineering with rolling features
    X, y, df_clean = feature_engineering_rolling(df)
    print(f"\nFeatures: {list(X.columns)}")
    print(f"Clean data points: {len(X)}")
    
    # Run model
    roi, acc, bets = run_temporal_cv(X, y, "TEMPORAL CV - ROLLING MODEL")
    
    # Plot
    if bets:
        plt.figure(figsize=(10, 5))
        cumsum = np.cumsum([b['PnL'] for b in bets])
        plt.plot(cumsum, color='purple', linewidth=2)
        plt.axhline(0, color='black', linestyle='--')
        plt.title(f'Cumulative Profit - Rolling Features Model (ROI: {roi:.1f}%)')
        plt.xlabel('Bet Number')
        plt.ylabel('Profit (Units)')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig('rolling_model_performance.png')
        print(f"\n[GRAPH] Saved to: rolling_model_performance.png")
